...
"""
Generate descriptors from ORCA output files with a flexible method functions for different energy calculations.
"""
...
def main():
    ...
    for ident in tqdm(identifiers):
        ...
        x_energy_file = f'./{x_file_prefix}/{x_file_prefix}/{x_file_prefix}_orca_{method}_energy.out'
        try:
        ...
        x_freq_file = f'./{x_file_prefix}/{x_file_prefix}/{x_file_prefix}_orca_freq.out'
        x_xyz_file = f'./{x_file_prefix}/{x_file_prefix}/{x_file_prefix}_.xtbopt.xyz'
        # x_atoms, x_num_atoms, x_coords = read_xyz_file(x_xyz_file)
        # x_i_index = x_atoms.index('I')
        # Analyse CuLpip structure
        nu_file_prefix = f'{ident}_CuLpip_1'
        nu_energy_file = f'./{nu_file_prefix}/{nu_file_prefix}/{nu_file_prefix}_orca_{method}_energy.out'
        nu_freq_file = f'./{nu_file_prefix}/{nu_file_prefix}/{nu_file_prefix}_orca_freq.out'
        nu_xyz_file = f'./{nu_file_prefix}/{nu_file_prefix}/{nu_file_prefix}.xtbopt.xyz'
        # Assuming piperidine was added to the structure last
        culpip_lignd_atoms = [atom + 1 for atom in coord_atoms]
        # Analyse TSOA structure
        tsoa_file_prefix = f'{ident}_TSOA_pip_1'
        tsoa_ligand_atoms = [atom + 29 for atom in coord_atoms]
        ...
        tsoa_energy_file = f'./{tsoa_file_prefix}/{tsoa_file_prefix}/{tsoa_file_prefix}_orca_{method}_energy.out'
        ...
        if tsoa_energy_data.metadata['success']:
            ...
            if len(tsoa_coordinates[0]) == (len(x_coordinates[0]) + 27):
                pass
            else:
                continue
            # Energy of the HOMO
        ...
        # Data from Freq file
        tsoa_freq_file = f'./{tsoa_file_prefix}/{tsoa_file_prefix}/{tsoa_file_prefix}_orca_freq.out'
        try:
        ...
        if tsoa_freq_data.metadata['success']:
            ...
            tsoa_xyz_file = f'./{tsoa_file_prefix}/{tsoa_file_prefix}/{tsoa_file_prefix}_orca_optts.xyz'
            tsoa_dummy_xyz_file = f'./{tsoa_file_prefix}/{tsoa_file_prefix}/{tsoa_file_prefix}_orca_optts_withDummy_NoSubstrates.xyz'
            ...
            if tsoa_midpoint is not None:
                ...
                complex_tsoa_atoms = [atom + 30 for atom in coord_atoms]
        ...
        tssig_file_prefix = f'{ident}_TSSig_pip_1'
        tssig_energy_file = f'./{tssig_file_prefix}/{tssig_file_prefix}/{tssig_file_prefix}_orca_{method}_energy.out'
        tssig_ligand_atoms = [atom + 29 for atom in coord_atoms]
        ...
        if tssig_energy_data.metadata['success']:
            ...
            if len(tssig_coordinates[0]) == 0:
                continue
            # Check to see if the length of atoms in the ligands is correct (some cases where A H is between two atoms and is deprotonated incorrectly as it can be considered bound to both)
            if len(tssig_coordinates[0]) == (len(x_coordinates[0]) + 27):
                pass
            else:
                continue
            # Energy of the HOMO
        ...
        # Data from Freq file
        tssig_freq_file = f'./{tssig_file_prefix}/{tssig_file_prefix}/{tssig_file_prefix}_orca_freq.out'
        try:
        ...
        if tssig_freq_data.metadata['success']:
            ...
            # Get data from the xyz file
            tssig_xyz_file = f'./{tssig_file_prefix}/{tssig_file_prefix}/{tssig_file_prefix}_orca_optts.xyz'
            tssig_dummy_xyz_file = f'./{tssig_file_prefix}/{tssig_file_prefix}/{tssig_file_prefix}_orca_optts_withDummy_NoSubstrates.xyz'
            # Literature Descriptors
            # Remove substrates, add a dummy H at the midpoint between the two designated coordinating atoms and write to a
            # new xyz file
            try:
            ...
            if tssig_midpoint is not None:
                ...
                # Define new coordinating atom numbers for ligand as module uses 1 as the start point for both the
                # Transition state complex and the complex with the dummy and substrates removed
                complex_tssig_atoms = [atom + 30 for atom in coord_atoms]
...
def write_dummy_structure(file, outfile, midpoint,):
    ...
    new_atoms = atoms[:1] + atoms[29:]
    new_num_atoms = num_atoms - 28
    new_coords = coords[:1] + coords[29:]
