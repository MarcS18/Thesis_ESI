...
def main():
    ...
    # Files headers
    headers = ['Ligand', 'Starting materials (CuLI) / kcal/mol', 'Ligand Exchange (CuLpip) / kcal/mol',
    ...
    outfile = 'Activation_Energies_TEST_ONLY.csv'
    ...
    for ident in tqdm(identifiers):
        ...
        # Else extract energy data
            ...
            if energy_data['sm_e'] == 'NaN' or energy_data['sm_e'] is None:
                ...
                sm = sm_g + gbl.dmf_common['piperidine']['B97-3c'] + gbl.dmf_common['iodobenzene']['B97-3c'] + \
                    2 * gbl.dmf_common['caesium']['B97-3c'] + gbl.dmf_common['carbonate']['B97-3c']
                product = sm_g + gbl.dmf_common['N_phenyl_piperidine']['B97-3c'] + \
