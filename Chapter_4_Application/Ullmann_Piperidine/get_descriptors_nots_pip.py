...
def main():
    ...
    nu_outfile = f'Structure_Analysis_Descriptors_CuLpip_{method}.csv'
    ...
    for ident in tqdm(identifiers):
        ...
        pip = ['N', 'C', 'C', 'C', 'C', 'C', 'H', 'H', 'H', 'H', 'H', 'H', 'H', 'H', 'H', 'H']
        nu_index = check_nu_position(nu_atom_list, pip)
        if nu_index == 1:
            nu_ligand_atoms = [atom + 17 for atom in coord_atoms]
        else:
            nu_ligand_atoms = [atom + 1 for atom in coord_atoms]
        n_index = nu_index
        culpip_atoms_of_interest = {'n': {'index': nu_index,
                                         'symbol': 'N',
        ...
        if nu_energy_data.metadata['success']:
            ...
            if len(nu_coordinates[0]) == (len(x_coordinates[0]) + 15):
                pass
            else:
                continue
            # Energy of the HOMO
        ...
        if nu_freq_data.metadata['success']:    
            ...
            nu_dummy_xyz_file = f'./{nu_file_prefix}/{nu_file_prefix}/{nu_file_prefix}_orca_optts_withDummy_NoSubstrates.xyz'
            ...
            if nu_midpoint is not None:
                ...
                if nu_index == 1:
                    complex_nu_atoms = [atom + 18 for atom in coord_atoms]
...
def write_dummy_structure(file, outfile, midpoint, nu_index):
    ...
    if nu_index == 1:
        new_atoms = atoms[:1] + atoms[17:]
        new_coords = coords[:1] + coords[17:]
    else:
        new_atoms = atoms[:-16]
        new_coords = coords[:-16]
