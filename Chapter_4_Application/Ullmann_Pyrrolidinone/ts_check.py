...
def main():
    ...
    # Output file headers
    headers = ['Ligand', 'Starting material (CuLI)', 'Ligand Exchange (CuLpyr)',
               'Transition State 1 (TSOA)', 'Transition State 2 (TSSig)']
    outfile = 'Structure_Analysis_Pyr.csv'
    ...
    for ident in tqdm(identifiers):
        freq_data = {'pyr_min': 'NaN', 'x_min': 'NaN', 'tsoa_ts': 'NaN', 'tssig_ts': 'NaN', 'ligand': ident}
        ...
        struc_path = f'{ident}_CuLpyr_1'
        pyr_min = perform_analysis(struc_path, mode='stable', tolerance=-20)
        freq_data.update({'pyr_min': pyr_min})
        # Analyse TSOA structure
        struc_path = f'{ident}_TSOA_pyr_1'
        tsoa_bonds = [[4, 13], [0, 4]]
        ...
        struc_path = f'{ident}_TSSig_pyr_1'
        tsssig_bond = [[1, 3]]
        ...
        struc_data = [freq_data['ligand'], freq_data['x_min'], freq_data['pyr_min'], freq_data['tsoa_ts'],
                      freq_data['tssig_ts']]
...
def perform_analysis(file_prefix, mode='stable', bonds=None, tolerance=0, ts_tolerance=-40, ts_freq_tolerance=0.33):
    ...
    :return:
    """
    freq_file = f'./{file_prefix}/{file_prefix}_orca_freq.out'
    xyz_file = f'./{file_prefix}/{file_prefix}_orca_optts.xyz'
    if mode == 'stable':
        try:
